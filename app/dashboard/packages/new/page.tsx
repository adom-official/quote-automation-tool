  ...
  return (
    <div className="h-full flex flex-col">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-3 mb-4 sm:mb-6 shrink-0">
        <div>
          <h2 className="text-xl sm:text-2xl font-bold text-slate-900 leading-snug">Xây dựng Gói Báo Giá Mới</h2>
          <p className="text-slate-500 text-xs sm:text-sm mt-1">Kéo thả hoặc nhấp chọn hạng mục từ thư viện để tự động tính toán ngân sách và thời gian.</p>
        </div>
        <div className="flex gap-2 items-center w-full sm:w-auto justify-end shrink-0">
          ...
          <button 
            onClick={handleSave} 
            disabled={isSaving}
            className="px-4 sm:px-5 py-2 bg-indigo-600 text-white rounded-xl text-xs sm:text-sm font-bold shadow-sm hover:bg-indigo-700 disabled:opacity-50 transition-colors whitespace-nowrap"
          >
      ...
      <div className="flex flex-col lg:flex-row flex-1 gap-4 lg:gap-6 overflow-y-auto lg:overflow-hidden min-h-0">
        ...
        <div className="w-full lg:w-80 flex flex-col border border-slate-200 rounded-2xl overflow-hidden shadow-2xs bg-white shrink-0 max-h-64 lg:max-h-none">
          ...
          <div className="flex-1 overflow-y-auto p-3 sm:p-4 space-y-2.5">
            {loadingItems ? (
              <p className="text-xs sm:text-sm text-slate-500 text-center py-4">Đang tải dữ liệu...</p>
            ) : items.length === 0 ? (
